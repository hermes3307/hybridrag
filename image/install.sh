...
# Image Recognition System - PostgreSQL + pgvector Installation Script
...
# Banner
echo "=========================================="
echo "Image Recognition System Installer"
...
DB_NAME="${POSTGRES_DB:-vector_images}"
...
if [ -f "schema.sql" ]; then
    ...
    TABLE_EXISTS=$(sudo -u postgres psql -d "$DB_NAME" -tAc "SELECT COUNT(*) FROM information_schema.tables WHERE table_name='images';")
...
# Step 12: Create images directory
log_info "Step 12: Creating images directory..."
if [ ! -d "images" ]; then
    mkdir -p images
    log_success "Images directory created"
else
    log_warning "Images directory already exists"
fi
...
log_success "Setup complete! Your image recognition system is ready to use."
