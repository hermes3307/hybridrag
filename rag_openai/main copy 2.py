...
class ConversationalAssistant:
    ...
    async def handle_index_intent(self, data: str) -> str:
        ...
        try:
            ...
            if not chunks:
                return ("🤖 I couldn't find the processed chunks. Please run chunking again.")
            ...
            if success:
                ...
                response += f"🔍 Collection: '{collection_info.get('name', 'conversation_docs')}'\n\n"
                response += "🎯 You can now search for information in your documents!"
    ...
    async def handle_search_intent(self, data: str) -> str:
        ...
        if not self.query_engine:
            return ("❌ Search engine is not available. Please install required packages:\n"
                   "pip install sentence-transformers qdrant-client")
        
        if not data or len(data.strip()) < 3:
            return ("🤖 What would you like me to search for? "
                   "Try something like: 'Find information about ipcda'")
        ...
        try:
            ...
            # Format response using the query engine's conversational formatter
            response = self.query_engine.format_conversational_response(data, results)
