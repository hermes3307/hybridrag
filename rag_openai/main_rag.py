...
Answer:"""
    ...
    def _generate_response_with_context(self, question: str, context: str, 
        ...
        technical_terms = extraction_result['technical_terms']
        
        # Get appropriate prompt template
        prompt_template = self.rag_prompts.get(question_type, self.rag_prompts['general'])
        
        # Determine main topic
        main_topic = ', '.join(keywords[:3]) if keywords else ', '.join(technical_terms[:2])
        
        # For now, create a structured response based on the context
        # In a real implementation, you'd send this to an LLM like GPT-4 or Claude
        response = self._create_structured_response(question, context, question_type, main_topic, results)
