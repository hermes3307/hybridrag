...
class EmbeddingManagerCLI:
    ...
    def get_database_stats(self) -> Dict:
        ...
        try:
            ...
            # Vectors by embedding model (multi-model support)
            # Count how many faces have each model's embedding
            cursor.execute("""
                SELECT
                    'facenet' as model, COUNT(*) as count FROM faces WHERE embedding_facenet IS NOT NULL
                UNION ALL
                SELECT 'arcface' as model, COUNT(*) as count FROM faces WHERE embedding_arcface IS NOT NULL
                UNION ALL
                SELECT 'vggface2' as model, COUNT(*) as count FROM faces WHERE embedding_vggface2 IS NOT NULL
                UNION ALL
                SELECT 'insightface' as model, COUNT(*) as count FROM faces WHERE embedding_insightface IS NOT NULL
                UNION ALL
                SELECT 'statistical' as model, COUNT(*) as count FROM faces WHERE embedding_statistical IS NOT NULL
                ORDER BY count DESC
            """)
            ...
            # Get embedded face_ids for the SPECIFIC model being used
            ...
            embedding_column = model_column_map.get(self.embedding_model.lower(), 'embedding_facenet')

            # Only get face_ids that have THIS model's embedding
            cursor.execute(f"SELECT face_id FROM faces WHERE {embedding_column} IS NOT NULL")
