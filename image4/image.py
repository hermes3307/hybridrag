...
class IntegratedImageGUI:
    ...
    def create_widgets(self):
        ...
        Creates a tabbed interface with six main tabs:
        ...
        - LLM Assistant: AI-powered natural language interface
        """
        ...
        # Tab 6: LLM Assistant - AI-powered command interface
        self.llm_frame = ttk.Frame(self.notebook)
        self.notebook.add(self.llm_frame, text="LLM Assistant")
        self.create_llm_tab()
    ...
    def create_search_tab(self):
        ...
        self.search_image_entry = ttk.Entry(control_frame, textvariable=self.search_image_var, width=20)
        self.search_image_entry.grid(row=0, column=1, sticky="w", padx=(5, 5))
        self.search_image_entry.bind("<FocusIn>", self.on_search_image_focus)
        ...
        self.search_text_entry = ttk.Entry(control_frame, textvariable=self.search_text_var, width=20)
        self.search_text_entry.grid(row=1, column=1, sticky="w", padx=(5, 5))
        self.search_text_entry.bind("<FocusIn>", self.on_search_text_focus)
    ...
    def create_config_tab(self):
        ...
        llm_config_frame = ttk.LabelFrame(self.config_frame, text="LLM Assistant Configuration", padding=10)
        llm_config_frame.grid(row=2, column=0, sticky="ew", padx=5, pady=5)

        # LLM Provider Selection
        ttk.Label(llm_config_frame, text="LLM Provider:").grid(row=0, column=0, sticky="w", pady=2)
        self.llm_provider_var = tk.StringVar(value="Simple Mode")
        llm_providers = ["Simple Mode", "Anthropic (Claude)", "OpenAI (GPT)", "Ollama (Local)"]
        self.llm_provider_combo = ttk.Combobox(llm_config_frame, textvariable=self.llm_provider_var,
                                               values=llm_providers, width=25, state="readonly")
        self.llm_provider_combo.grid(row=0, column=1, sticky="w", padx=(5, 10))
        self.llm_provider_combo.bind('<<ComboboxSelected>>', self.on_llm_provider_changed)

        # Model Selection (changes based on provider)
        ttk.Label(llm_config_frame, text="Model:").grid(row=1, column=0, sticky="w", pady=2)
        self.llm_model_var = tk.StringVar(value="Pattern Matching")
        self.llm_model_combo = ttk.Combobox(llm_config_frame, textvariable=self.llm_model_var,
                                            values=["Pattern Matching"], width=25, state="readonly")
        self.llm_model_combo.grid(row=1, column=1, sticky="w", padx=(5, 10))

        # API Key Entry
        ttk.Label(llm_config_frame, text="API Key:").grid(row=2, column=0, sticky="w", pady=2)
        self.llm_api_key_var = tk.StringVar(value="")
        self.llm_api_key_entry = ttk.Entry(llm_config_frame, textvariable=self.llm_api_key_var,
                                           width=40, show="*")
        self.llm_api_key_entry.grid(row=2, column=1, sticky="w", padx=(5, 10))

        # Show/Hide API Key button
        self.llm_show_key_btn = ttk.Button(llm_config_frame, text="Show",
                                           command=self.toggle_llm_api_key_visibility, width=8)
        self.llm_show_key_btn.grid(row=2, column=2, padx=5)

        # API Key Status
        self.llm_api_status_label = ttk.Label(llm_config_frame, text="No API key configured",
                                              foreground="orange", font=('TkDefaultFont', 9))
        self.llm_api_status_label.grid(row=3, column=0, columnspan=3, sticky="w", pady=(5, 0))

        # LLM Configuration Buttons
        llm_button_frame = ttk.Frame(llm_config_frame)
        llm_button_frame.grid(row=4, column=0, columnspan=3, pady=(10, 0))

        ttk.Button(llm_button_frame, text="Test LLM Connection",
                  command=self.test_llm_connection).pack(side="left", padx=5)
        ttk.Button(llm_button_frame, text="Save API Key to .env",
                  command=self.save_llm_api_key).pack(side="left", padx=5)
        ttk.Button(llm_button_frame, text="Load from .env",
                  command=self.load_llm_from_env).pack(side="left", padx=5)
        ttk.Button(llm_button_frame, text="Apply & Reinitialize",
                  command=self.apply_llm_config).pack(side="left", padx=5)

        # Load current LLM config from environment
        self.load_llm_from_env()
    ...
    def create_llm_tab(self):
        """Create LLM Assistant tab"""

        # Configure grid weights
        self.llm_frame.columnconfigure(0, weight=1)
        self.llm_frame.rowconfigure(1, weight=1)

        # Header frame with info and controls
        header_frame = ttk.LabelFrame(self.llm_frame, text="LLM Assistant", padding=10)
        header_frame.grid(row=0, column=0, sticky="ew", padx=5, pady=5)

        info_text = "Use natural language to control the image processing system. You can ask the assistant to download images, process embeddings, search for images, and more."
        ttk.Label(header_frame, text=info_text, wraplength=800, justify="left").grid(row=0, column=0, columnspan=3, sticky="w", pady=(0, 10))

        # API Status
        ttk.Label(header_frame, text="API Status:", font=('TkDefaultFont', 9, 'bold')).grid(row=1, column=0, sticky="w")
        self.llm_status_label = ttk.Label(header_frame, text="Not initialized", foreground="orange")
        self.llm_status_label.grid(row=1, column=1, sticky="w", padx=(5, 0))

        # Background operations status
        ttk.Label(header_frame, text="Operations:", font=('TkDefaultFont', 9, 'bold')).grid(row=2, column=0, sticky="w", pady=(5, 0))
        self.llm_operations_label = ttk.Label(header_frame, text="Idle", foreground="gray")
        self.llm_operations_label.grid(row=2, column=1, sticky="w", padx=(5, 0), pady=(5, 0))

        # Control buttons
        ttk.Button(header_frame, text="Clear Chat", command=self.clear_llm_chat).grid(row=1, column=2, rowspan=2, padx=5, sticky="e")

        # Chat display frame
        chat_frame = ttk.LabelFrame(self.llm_frame, text="Conversation", padding=10)
        chat_frame.grid(row=1, column=0, sticky="nsew", padx=5, pady=5)
        chat_frame.columnconfigure(0, weight=1)
        chat_frame.rowconfigure(0, weight=1)

        # Chat history display
        self.llm_chat_display = scrolledtext.ScrolledText(chat_frame, height=20, width=100, wrap=tk.WORD, state='disabled')
        self.llm_chat_display.grid(row=0, column=0, sticky="nsew", padx=5, pady=5)

        # Configure tags for styling
        self.llm_chat_display.tag_config("user", foreground="blue", font=('TkDefaultFont', 9, 'bold'))
        self.llm_chat_display.tag_config("assistant", foreground="green", font=('TkDefaultFont', 9, 'bold'))
        self.llm_chat_display.tag_config("system", foreground="orange", font=('TkDefaultFont', 9, 'italic'))
        self.llm_chat_display.tag_config("error", foreground="red", font=('TkDefaultFont', 9))
        ...
        # User input
        ttk.Label(input_frame, text="Your message:").grid(row=0, column=0, sticky="w", pady=(0, 5))

        self.llm_input_text = scrolledtext.ScrolledText(input_frame, height=3, width=80, wrap=tk.WORD)
        self.llm_input_text.grid(row=1, column=0, sticky="ew", padx=(0, 5))
        self.llm_input_text.bind('<Control-Return>', lambda e: self.send_llm_message())

        # Send button
        self.llm_send_button = ttk.Button(input_frame, text="Send Message", command=self.send_llm_message)
        self.llm_send_button.grid(row=1, column=1, padx=(0, 5))

        # Examples frame
        examples_frame = ttk.LabelFrame(self.llm_frame, text="Example Commands", padding=10)
        examples_frame.grid(row=3, column=0, sticky="ew", padx=5, pady=5)

        examples_text = """Try these commands:
• "Download 20 landscape images"
• "Process all new images and generate embeddings"
• "Search for images similar to /path/to/image.jpg"
• "Show me the current system status"
• "Setup the database for vector search"
"""
        ttk.Label(examples_frame, text=examples_text, justify="left", font=('TkDefaultFont', 9)).pack(anchor="w")
    ...
    def initialize_llm_assistant(self):
        """Initialize the LLM assistant with API key from environment"""
        try:
            # Try to import the multi-provider assistant
            from llm_assistant_multi import create_llm_assistant

            # Get provider and configuration from environment
            ...
            # Create assistant
            self.llm_assistant = create_llm_assistant(
                provider=provider,
                api_key=api_key,
                model=model if model else None
            )

            # Update status label based on provider
            provider_names = {
                'anthropic': f"Connected (Claude {model or 'API'})",
                'openai': f"Connected (GPT {model or 'API'})",
                'ollama': f"Connected (Ollama {model or 'Local'})",
                'simple': "Simple mode (Pattern matching)"
            }

            status_text = provider_names.get(provider, "Connected")
            status_color = "green" if provider != 'simple' else "orange"

            self.llm_status_label.config(text=status_text, foreground=status_color)

            # Welcome message
            if provider == 'simple':
                self.append_llm_message("system", "LLM Assistant initialized in simple mode. Configure API keys in the Configuration tab for full AI capabilities.\n\nI can still help with basic commands using pattern matching.")
            else:
                self.append_llm_message("system", f"LLM Assistant initialized with {provider_names[provider]}. How can I help you?")

            # Register available commands
            self.register_llm_commands()

        except ImportError as e:
            self.llm_status_label.config(text="Error - Module not found", foreground="red")
            self.append_llm_message("error", f"Failed to initialize LLM assistant: {str(e)}\nMake sure llm_assistant_multi.py is present.")
        except Exception as e:
            self.llm_status_label.config(text="Error - Initialization failed", foreground="red")
            self.append_llm_message("error", f"Failed to initialize LLM assistant: {str(e)}")

    def register_llm_commands(self):
        """Register commands that the LLM can execute"""
        if not self.llm_assistant:
            return

        # Download images command
        self.llm_assistant.register_command(
            "download_images",
            "Download images from a specified source",
            self.llm_download_images
        )

        # Embed all images command
        self.llm_assistant.register_command(
            "embed_images",
            "Process and embed all images in the database",
            self.llm_embed_all_images
        )

        # Embed new images command
        self.llm_assistant.register_command(
            "embed_new_images",
            "Process and embed only new (unprocessed) images",
            self.llm_embed_new_images
        )
        ...
        self.llm_assistant.register_command(
            ...
            self.llm_setup_database
        )

    def send_llm_message(self):
        """Send a message to the LLM assistant"""
        if not self.llm_assistant or self.llm_processing:
            return

        user_message = self.llm_input_text.get("1.0", tk.END).strip()
        if not user_message:
            return
        ...
        self.append_llm_message("user", user_message)

        # Process in background thread
        self.llm_processing = True
        self.llm_send_button.config(state='disabled', text="Processing...")
        threading.Thread(target=self._process_llm_message, args=(user_message,), daemon=True).start()

    def _process_llm_message(self, user_message: str):
        """Process LLM message in background thread"""
        try:
            # Update system status before processing
            if self.llm_assistant and hasattr(self.llm_assistant, 'update_system_status'):
                status = self.get_current_system_status()
                self.llm_assistant.update_system_status(status)

            # Get response from LLM
            response = self.llm_assistant.chat(user_message)
            ...
            self.root.after(0, self.append_llm_message, "error", error_msg)
            self.root.after(0, self._reset_llm_input)

    def _handle_llm_response(self, response: dict):
        """Handle LLM response in main thread"""
        # Display assistant's response
        if response.get('error'):
            self.append_llm_message("error", response.get('response', 'Unknown error'))
        else:
            self.append_llm_message("assistant", response.get('response', ''))

        # Reset input IMMEDIATELY so user can continue chatting
        self._reset_llm_input()

        # Execute action if present (in background, doesn't block chat)
        if response.get('action'):
            ...
            if explanation:
                self.append_llm_message("system", f"Executing: {explanation}")

            # Execute the action (this won't block the chat)
            result = self.llm_assistant.execute_action(action, parameters)

            # Show result message (action handlers manage their own progress reporting)
            if result.get('success'):
                result_msg = result.get('result', 'Done')
                # Only show completion message if it's not a background operation
                if result_msg and not result_msg.startswith("Starting") and not result_msg.startswith("Processing started"):
                    self.append_llm_message("system", f"✓ {result_msg}")
            else:
                self.append_llm_message("error", f"Action failed: {result.get('error', 'Unknown error')}")

    def _reset_llm_input(self):
        """Reset LLM input controls"""
        self.llm_processing = False
        self.llm_send_button.config(state='normal', text="Send Message")

    def append_llm_message(self, sender: str, message: str):
        """Append a message to the LLM chat display

        Args:
            sender: 'user', 'assistant', 'system', or 'error'
            message: The message text
        """
        self.llm_chat_display.config(state='normal')
        ...
        if sender == "user":
            self.llm_chat_display.insert(tk.END, f"[{timestamp}] You: ", "user")
            self.llm_chat_display.insert(tk.END, f"{message}\n\n")
        elif sender == "assistant":
            self.llm_chat_display.insert(tk.END, f"[{timestamp}] Assistant: ", "assistant")
            self.llm_chat_display.insert(tk.END, f"{message}\n\n")
        elif sender == "system":
            self.llm_chat_display.insert(tk.END, f"[{timestamp}] System: ", "system")
            self.llm_chat_display.insert(tk.END, f"{message}\n\n", "system")
        elif sender == "error":
            self.llm_chat_display.insert(tk.END, f"[{timestamp}] Error: ", "error")
            self.llm_chat_display.insert(tk.END, f"{message}\n\n", "error")

        self.llm_chat_display.config(state='disabled')
        self.llm_chat_display.see(tk.END)

    def clear_llm_chat(self):
        """Clear the LLM chat history"""
        if self.llm_assistant:
            self.llm_assistant.clear_history()

        self.llm_chat_display.config(state='normal')
        self.llm_chat_display.delete("1.0", tk.END)
        self.llm_chat_display.config(state='disabled')
        self.append_llm_message("system", "Chat cleared. How can I help you?")
    ...
    def _update_operations_status(self, status_text: str, color: str = "gray"):
        ...
        Args:
            ...
            color: Color for the text (gray, blue, green, orange, red)
        """
    ...
    def get_current_system_status(self) -> dict:
        ...
        try:
            status = {
                'database_connected': self.system is not None,
                'images_directory': self.images_dir_var.get(),
    ...
    def llm_download_images(self, count: int = 10, source: str = "picsum_landscape") -> str:
        """Download images via LLM command with detailed progress reporting"""
        try:
            # Update GUI settings
            self.download_source_var.set(source)
            ...
            threading.Thread(
                target=self._llm_download_images_with_progress,
                args=(count, source),
                daemon=True
            ).start()

            return f"Starting download of {count} images from {source}. Watch for progress updates below..."
        except Exception as e:
            return f"Error: {str(e)}"

    def _llm_download_images_with_progress(self, count: int, source: str):
        """Background thread to download images with progress reporting"""
        if not self.system:
            self.root.after(0, self.append_llm_message, "error", "System not initialized")
            return

        # Update operations status
        self.root.after(0, self._update_operations_status, f"Downloading {count} images...", "blue")

        self.root.after(0, self.append_llm_message, "system", f"📥 Download started: {count} images from {source}")
        ...
        for i in range(count):
            try:
                ...
                self.root.after(0, self._update_operations_status, f"Downloading [{i+1}/{count}]...", "blue")
                ...
                if file_path:
                    ...
                    self.root.after(0, self.append_llm_message, "system", progress_msg)

                    # Update download thumbnail in GUI
                    self.root.after(0, self.add_download_thumbnail, file_path)
                else:
                    duplicates += 1
...
Total processed: {count}
        """
        self.root.after(0, self.append_llm_message, "system", summary.strip())

        # Reset operations status
        self.root.after(0, self._update_operations_status, "Idle", "gray")

    def llm_embed_all_images(self) -> str:
        """Process and embed all images via LLM command with progress reporting"""
        try:
            # Switch to process tab
            self.notebook.select(self.process_frame)

            # Update operations status
            self._update_operations_status("Processing all images...", "blue")

            # Report start
            self.append_llm_message("system", "🔄 Starting to process and embed all images...")

            # Trigger processing
            self.root.after(0, self.start_processing)

            return "Processing started. This may take a while depending on the number of images. Progress will be shown in the Process & Embed tab."
        except Exception as e:
            return f"Error: {str(e)}"

    def llm_embed_new_images(self) -> str:
        """Process and embed new images via LLM command with progress reporting"""
        try:
            # Switch to process tab
            self.notebook.select(self.process_frame)

            # Update operations status
            self._update_operations_status("Processing new images...", "blue")

            # Report start
            self.append_llm_message("system", "🔄 Starting to process and embed new (unprocessed) images...")

            # Trigger processing
            self.root.after(0, self.process_new_images)

            return "Processing started for new images only. Progress will be shown in the Process & Embed tab."
        except Exception as e:
            return f"Error: {str(e)}"

    def llm_search_images(self, query_image: str, limit: int = 10) -> str:
        """Search for similar images via LLM command"""
        try:
            # Switch to search tab
            self.notebook.select(self.search_frame)

            # Update search parameters
            # Note: This would require implementing image path selection
            # For now, just switch to the tab

            self.append_llm_message("system", f"🔍 Switched to Search tab. Please select a query image to find {limit} similar images.")

            return f"Ready to search. Please:\n1. Click 'Browse' to select a query image\n2. Or drag an image to the preview area\n3. Click 'Search' to find similar images"
        except Exception as e:
            return f"Error: {str(e)}"

    def llm_get_status(self) -> str:
        """Get system status via LLM command"""
        try:
            status = self.get_current_system_status()

            # Format status nicely
            status_lines = []
            status_lines.append("📊 Current System Status")
            status_lines.append("━━━━━━━━━━━━━━━━━━━━")

            for key, value in status.items():
                # Format key nicely
                formatted_key = key.replace('_', ' ').title()
                status_lines.append(f"  {formatted_key}: {value}")
    ...
    def llm_setup_database(self) -> str:
        """Setup database via LLM command"""
        try:
            # Switch to config tab
            self.notebook.select(self.config_frame)

            self.append_llm_message("system", "⚙️ Initializing vector database...")

            # Trigger database initialization
            self.root.after(0, self.initialize_vector_database)

            return "Database initialization started. This will:\n1. Create database tables\n2. Enable pgvector extension\n3. Set up vector indexes\n\nCheck the System Overview tab for completion status."
        except Exception as e:
            return f"Error: {str(e)}"
    ...
    def on_llm_provider_changed(self, event=None):
        ...
        if provider == "Simple Mode":
            models = ["Pattern Matching"]
            self.llm_api_key_entry.config(state='disabled')
        elif provider == "Anthropic (Claude)":
            models = ["claude-sonnet-4-20250514", "claude-3-5-sonnet-20241022", "claude-3-opus-20240229"]
            self.llm_api_key_entry.config(state='normal')
        ...
        self.llm_model_combo.config(values=models)
        self.llm_model_var.set(models[0])

    def toggle_llm_api_key_visibility(self):
        """Toggle API key visibility"""
        if self.llm_api_key_entry.cget('show') == '*':
            self.llm_api_key_entry.config(show='')
            self.llm_show_key_btn.config(text='Hide')
        else:
            self.llm_api_key_entry.config(show='*')
            self.llm_show_key_btn.config(text='Show')

    def load_llm_from_env(self):
        ...
        try:
            ...
            llm_provider = os.getenv('LLM_PROVIDER', 'simple')
            llm_model = os.getenv('LLM_MODEL', '')

            # Set provider based on available keys or config
            if llm_provider == 'anthropic' or anthropic_key:
                self.llm_provider_var.set("Anthropic (Claude)")
                self.llm_api_key_var.set(anthropic_key)
                if llm_model:
                    self.llm_model_var.set(llm_model)
            elif llm_provider == 'openai' or openai_key:
                self.llm_provider_var.set("OpenAI (GPT)")
                self.llm_api_key_var.set(openai_key)
                if llm_model:
                    self.llm_model_var.set(llm_model)
            elif llm_provider == 'ollama' or ollama_host:
                self.llm_provider_var.set("Ollama (Local)")
                if llm_model:
                    self.llm_model_var.set(llm_model)
            else:
                self.llm_provider_var.set("Simple Mode")
            ...
            if anthropic_key or openai_key:
                self.llm_api_status_label.config(text="API key loaded from .env", foreground="green")
            elif ollama_host:
                self.llm_api_status_label.config(text="Ollama configuration loaded", foreground="green")
            else:
                self.llm_api_status_label.config(text="Using simple mode (no API key)", foreground="orange")

        except Exception as e:
            self.llm_api_status_label.config(text=f"Error loading config: {str(e)}", foreground="red")

    def save_llm_api_key(self):
        """Save LLM API key to .env file"""
        try:
            provider = self.llm_provider_var.get()
            api_key = self.llm_api_key_var.get()
            model = self.llm_model_var.get()

            if not api_key and provider not in ["Simple Mode", "Ollama (Local)"]:
            ...
            if provider == "Anthropic (Claude)":
                keys_to_update['ANTHROPIC_API_KEY'] = api_key
                keys_to_update['LLM_PROVIDER'] = 'anthropic'
                keys_to_update['LLM_MODEL'] = model
            elif provider == "OpenAI (GPT)":
                keys_to_update['OPENAI_API_KEY'] = api_key
                keys_to_update['LLM_PROVIDER'] = 'openai'
                keys_to_update['LLM_MODEL'] = model
            elif provider == "Ollama (Local)":
                keys_to_update['LLM_PROVIDER'] = 'ollama'
                keys_to_update['LLM_MODEL'] = model
                keys_to_update['OLLAMA_HOST'] = 'http://localhost:11434'
            else:
                keys_to_update['LLM_PROVIDER'] = 'simple'
            ...
            self.llm_api_status_label.config(text="Configuration saved to .env", foreground="green")
            messagebox.showinfo("Success", "LLM configuration saved to .env file.\n\nClick 'Apply & Reinitialize' to activate the new settings.")

        except Exception as e:
            self.llm_api_status_label.config(text=f"Error saving: {str(e)}", foreground="red")
    ...
    def test_llm_connection(self):
        ...
        try:
            ...
            if provider == "Simple Mode":
                messagebox.showinfo("Test Result", "Simple mode doesn't require a connection.\n\nPattern matching is always available.")
                return

            # Test connection based on provider
            if provider == "Anthropic (Claude)":
                if not api_key:
                    messagebox.showerror("Error", "Please enter an Anthropic API key")
                    return
                ...
                messagebox.showinfo("Success", f"✓ Connected to Anthropic Claude!\n\nModel: {model}\nResponse: {response.content[0].text[:50]}...")
                ...
                messagebox.showinfo("Success", f"✓ Connected to OpenAI GPT!\n\nModel: {model}\nResponse: {response.choices[0].message.content[:50]}...")
                ...
                try:
                    ...
                    if response.status_code == 200:
                        messagebox.showinfo("Success", f"✓ Connected to Ollama!\n\nModel: {model}\nHost: {ollama_host}")
                    else:
                        messagebox.showerror("Error", f"Ollama connection failed: {response.text}")
            ...
            if "Connection refused" in error_msg or "Max retries" in error_msg:
                if provider == "Ollama (Local)":
                    messagebox.showerror("Ollama Not Running",
                        ...
                        f"Or visit: https://ollama.ai/")
                else:
                    messagebox.showerror("Connection Error",
                        f"Cannot connect to {provider}:\n\n{error_msg}\n\n"
                        "Please check your internet connection.")
            elif "API key" in error_msg or "Incorrect API key" in error_msg:
    ...
    def apply_llm_config(self):
        """Apply LLM configuration and reinitialize the assistant"""
        try:
            provider = self.llm_provider_var.get()
            api_key = self.llm_api_key_var.get()
            model = self.llm_model_var.get()
            ...
            if provider == "Anthropic (Claude)":
                ...
                os.environ['LLM_PROVIDER'] = 'simple'

            # Reinitialize the LLM assistant
            self.initialize_llm_assistant()

            self.llm_api_status_label.config(text="Configuration applied successfully", foreground="green")
            messagebox.showinfo("Success", f"LLM Assistant reinitialized with {provider}\n\nModel: {model}\n\nGo to the LLM Assistant tab to test it!")

        except Exception as e:
            self.llm_api_status_label.config(text=f"Error applying config: {str(e)}", foreground="red")
    ...
    def browse_search_image(self):
        ...
        if file_path:
            ...
            # Switch to vector mode when image is selected
            if self.search_mode_var.get() != "vector":
                self.search_mode_var.set("vector")
    ...
    def on_search_text_focus(self, event):
        """Handle focus event on search text entry - switch to Text search mode"""
        if self.search_mode_var.get() != "text":
            self.search_mode_var.set("text")
            self.on_search_mode_changed()

    def on_search_image_focus(self, event):
        """Handle focus event on search image entry - switch to Vector search mode"""
        if self.search_mode_var.get() != "vector":
            self.search_mode_var.set("vector")
            self.on_search_mode_changed()
