...
class CaseStudyLLM:
    ...
    def generate_prompt(self, 
        ...
        Args:
            ...
            max_similar_cases: 포함할 최대 유사 케이스 수 (토큰 제한을 위해)
            estimate_tokens: 토큰 수 예측 여부
        ...
        # 유사 케이스 스터디
        if limited_cases:
            prompt += f"다음은 참고할 수 있는 유사한 케이스 스터디들입니다 (상위 {len(limited_cases)}개):\n\n"
        ...
        # 토큰 수 예측 (대략적인 추정)
        if estimate_tokens:
            # 영어 기준 대략 1토큰 = 4글자, 한글은 더 많이 사용
            estimated_tokens = len(prompt) / 2  # 한글이 많으므로 2글자당 1토큰으로 대략 계산
            print(f"예상 토큰 수: 약 {int(estimated_tokens)}개")
            
            # 토큰 제한 경고
            if self.model_type == "llama" and estimated_tokens > 3500:  # 여유를 두고 3500으로 설정
                print(f"경고: 프롬프트가 너무 깁니다. 컨텍스트 제한(4096 토큰)을 초과할 수 있습니다.")
                print(f"유사 케이스 수를 {max_similar_cases-1}개 또는 그 이하로 줄이는 것을 고려하세요.")
    ...
    def generate_response(self, prompt: str, max_tokens: int = 2048) -> str:
        ...
        try:
            if self.model_type == "llama":
                ...
                # 토큰 수 계산 시도
                try:
                    # llama_cpp에는 내장된 토큰 카운터가 있을 수 있음
                    input_tokens = self.model.tokenize(prompt.encode("utf-8"))
                    input_token_count = len(input_tokens)
                    print(f"입력 프롬프트 토큰 수: {input_token_count}")
                    
                    # 컨텍스트 크기 확인
                    n_ctx = self.model.n_ctx
                    print(f"모델 컨텍스트 크기: {n_ctx}")
                    
                    # 출력 가능한 최대 토큰 수 계산
                    available_tokens = max(0, n_ctx - input_token_count)
                    print(f"생성 가능한 최대 토큰 수: {available_tokens}")
                    
                    # 토큰 수 조정
                    if available_tokens < max_tokens:
                        print(f"경고: 요청한 생성 토큰 수({max_tokens})가 가용 토큰 수({available_tokens})보다 큽니다.")
                        print(f"생성 토큰 수를 {available_tokens}으로 조정합니다.")
                        max_tokens = available_tokens
                except Exception as e:
                    print(f"토큰 수 계산 중 오류: {str(e)}")
                
                # 토큰 수가 컨텍스트 크기를 초과하는 경우
                if max_tokens <= 0:
                    return "오류: 프롬프트가 너무 깁니다. 유사 케이스 수를 줄이거나 프롬프트를 수정하세요."
                ...
                try:
                    ...
                    # 일반적인 Gemma 모델의 컨텍스트 크기 (모델마다 다를 수 있음)
                    if hasattr(self.model.config, "max_position_embeddings"):
                        n_ctx = self.model.config.max_position_embeddings
                    else:
                        n_ctx = 8192  # 기본값
                    print(f"모델 컨텍스트 크기: {n_ctx}")
                    ...
                    if available_tokens < max_tokens:
                        print(f"경고: 요청한 생성 토큰 수({max_tokens})가 가용 토큰 수({available_tokens})보다 큽니다.")
                        print(f"생성 토큰 수를 {available_tokens}으로 조정합니다.")
                        max_tokens = min(available_tokens, 1024)  # 안전을 위해 최대 1024로 제한
                except Exception as e:
                    print(f"토큰 수 계산 중 오류: {str(e)}")
